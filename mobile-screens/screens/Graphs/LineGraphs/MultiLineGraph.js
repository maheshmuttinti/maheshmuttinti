...
import {getMultipleLineGraphData} from 'services';
import {useWindowDimensions} from 'react-native';

export default function () {
  const multipleLineGraphData = getMultipleLineGraphData();
    ...
    <ScrollView style={{paddingTop: 55}}>
      <View style={{flex: 1, paddingBottom: 100, paddingHorizontal: 24}}>
        <View style={{paddingVertical: 12}}>
          <Heading title="Multi Line Graph" />
        </View>
        <View>
          <SkiaD3LineGraph
            data={multipleLineGraphData}
            yAxisLine={false}
            xAxisLine={true}
